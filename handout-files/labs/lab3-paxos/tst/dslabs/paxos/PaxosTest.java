...
import static dslabs.paxos.Log.majority;
import static dslabs.paxos.Log.slotStatus;
...
import static dslabs.paxos.Log.partitionMsg;
...
public class PaxosTest extends BaseJUnitTest {
            ...
            .statePredicateWithMessage(
                    "First non-cleared and last non-empty valid", st -> {
                        for (Node n : st.servers()) {
                            ...
                            if (slotStatus) Log.info("StatePredicate " + p);
    ...
    private static Pair<Boolean, String> slotValid(AbstractState st, int i) {
        ...
        for (Node n : st.servers()) {
            ...
            if (majority) {
                Log.info(p + " slot is " + i + " status is " + s + " command is " + c);
            }
            ...
            try {
                while (!Thread.interrupted()) {
                    for (int i = 0; i < 2; i++) {
                        ...
                        if (partitionMsg) {
                            Log.info(i + " : partition " + newPartition);
                        }
                    ...
                    if (partitionMsg) Log.info("partition heal");
